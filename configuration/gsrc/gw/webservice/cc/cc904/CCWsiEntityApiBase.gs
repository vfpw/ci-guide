...
abstract class CCWsiEntityApiBase {
  ...
  protected function assembleFromDTO(dto: ClaimDTO, policy: Policy): Claim {
    var claim    = dto.writeToNewEntityIn(policy)
    ...
    if (dto.NewLossLocation != null) {
      claim.LossLocation = assembleFromDTO(dto.NewLossLocation, policy)
    }

    assembleFromDTOs(dto.NewContacts,   policy, \ctc -> {ctc.Claim = claim})
    assembleFromDTOs(dto.NewDocuments,  policy, \doc -> {doc.Claim = claim})
    assembleFromDTOs(dto.NewIncidents,  policy, \inc -> {inc.Claim = claim})
    assembleFromDTOs(dto.NewNotes,      policy, \nte -> {nte.Claim = claim})
  ...
  protected function assembleFromDTO(dto: ClaimDTO, bundleProvider: BundleProvider, policyDTO: PolicyDTO): Claim {
    ...
    var claim  = assembleFromDTO(dto, policy)
    return claim
  }
  ...
  protected function assembleFromDTO(dto: ClaimDTO, bundleProvider: BundleProvider, policySummaryDTO: PolicySummaryDTO): Claim {
    ...
    policy = bundleProvider.Bundle.add(policy)
    var claim = assembleFromDTO(dto, policy)
