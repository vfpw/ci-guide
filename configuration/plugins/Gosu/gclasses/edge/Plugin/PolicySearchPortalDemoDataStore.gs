...
uses gw.plugin.policy.impl.PolicySearchDemoDataStore

class PolicySearchPortalDemoDataStore extends PolicySearchDemoDataStore {

  private var _policyGenerator : block() : Policy[]
  private var _policies : Policy[]

  construct(policyGenerator : block() : Policy[]) {

     super(\ -> new PolicySearchPolicyGenerator().generatePolicies())
     _policyGenerator = policyGenerator
     generatePolicies()
   }

  public override function getPolicy( policyNumber : String, lossDate : Date ) : Policy {
  ...
  public override function findPolicies( criteria : PolicySearchCriteria ) : Policy[] {
  ...
  private function policyMatchesCriteria( policy : Policy, criteria : PolicySearchCriteria ) : boolean {
    ...
    var isMatch = lossDateMatches and
                  ...
                  ( !criteria.PolicyNumber.HasContent or ( policy.PolicyNumber != null and policy.PolicyNumber.startsWith( criteria.PolicyNumber ) ) ) and
                  ( !criteria.ProducerCode.HasContent or policy.ProducerCode == criteria.ProducerCode) and
                  ( !criteria.AccountNumber.HasContent or policy.AccountNumber == criteria.AccountNumber )
    if( isMatch ) {
      ...
      if( criteria.CompanyName.HasContent ) {
        isMatch = (insured != null) and 
                  ...
                  compareString( criteria.CompanyName, insured.Name )
      }
  ...
  private function compareString( enteredValue : String, storedValue : String) : boolean {
    ...
    protected function generatePolicies()  {
     ...
     var policies = _policyGenerator()
     for(var p in policies) {
       var number = Integer.parseInt(p.PolicyNumber.replaceAll("-",""))
       p.ProducerCode = (number % 2) == 0 ? "123" : "321"
     }

     var policiesList = policies.toList()
     policiesList.addAll(getExtraPolicies(policies[0].Bundle).toList())
     _policies = policiesList.toTypedArray()
   }
