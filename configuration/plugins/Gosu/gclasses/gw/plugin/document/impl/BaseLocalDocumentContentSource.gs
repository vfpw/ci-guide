...
abstract class BaseLocalDocumentContentSource implements InitializablePlugin
{
  ...
  //Deprecated - Determine whether a hidden frame should be targeted
  private static final var TARGET_PARAM = "target";
  ...
  override function setParameters( parameters : Map ) : void {
    if (parameters != null) {
      ...
      if (target.HasContent) {
        if (contentDispositionTypeName.HasContent) {
          PLLoggerCategory.DOCUMENT.warn("DocumentContentSource contentDisposition supercedes deprecated target=" + target);
        } else {
          _contentDispositionType = "hidden".compareToIgnoreCase(target) == 0 ?
              ContentDispositionType.ATTACHMENT : ContentDispositionType.INLINE;
          PLLoggerCategory.DOCUMENT.warn("DocumentContentSource specifies deprecated target=" + target +
              ", use contentDisposition=" + _contentDispositionType.name().toLowerCase() + " instead");
        }
      }
    }
  }
